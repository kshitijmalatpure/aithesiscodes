...
class ComprehensiveMLPipeline:
    ...
    # In your new ComprehensiveMLPipeline class
    ...
    def run_single_experiment(self, fold_num, model_name, feature_selection_name):
        ...
        try:
            ...
            # --- CRITICAL FIX: Handle NaNs created by StandardScaler ---
            # This prevents crashes when a feature has zero variance in the training set.
            X_train_scaled = np.nan_to_num(X_train_scaled, nan=0.0, posinf=0.0, neginf=0.0)
            X_test_scaled = np.nan_to_num(X_test_scaled, nan=0.0, posinf=0.0, neginf=0.0)
            # --- END OF FIX ---

            pca_transformer = None
            if feature_selection_name.startswith('PCA'):
                n_components = int(feature_selection_name.split('_')[1])
                pca_transformer = PCA(n_components=min(n_components, X_train_scaled.shape[1]), random_state=42)
            ...
            for i, score_col in enumerate(self.score_columns):
                ...
                if feature_selection_name == 'Baseline':
                    ...
                    X_train_selected = pca_transformer.transform(X_train_scaled)
                    X_test_selected = pca_transformer.transform(X_test_scaled)
    ...
    # can remain the same. They will correctly process the results dictionary which now includes
    # metrics calculated on the SMOTE-balanced training data.
    ...
    def save_results(self):
        # This function does not need changes
        results_dir = self.base_dir / "comprehensive_results_regression_binary"
    ...
    def create_visualizations(self):
        ...
        # --- START: CORRECTED METRIC GROUPS DICTIONARY ---
        # Each metric now has a dictionary as its value, ensuring consistent structure.
        metric_groups = {
            'regression_metrics': {
        ...
        # --- END: CORRECTED METRIC GROUPS DICTIONARY ---
            ...
            for score_col in score_cols_to_plot:
                ...
                for metric, properties in metrics_to_plot.items():
                    ...
                    try:
                        ...
                        # Explicitly choose the color map based on the metric's property
...
def main():
    ...
    print(f"Check the '{pipeline.base_dir / 'comprehensive_results_regression_binary'}' directory for all outputs.")
