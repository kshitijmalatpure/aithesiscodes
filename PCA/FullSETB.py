...
class ComprehensiveMLPipeline:
    def __init__(self, base_dir, name_column, score_columns, labels):
        ...
        # Structure: model -> fs_method -> score_col -> list_of_fold_data
        ...
        # Models are now single-target classifiers
        self.models = {
        ...
        self.feature_selections = {
            ...
            'LASSO': 'lasso'  # Special handling for LASSO
        }
    ...
    def apply_lasso_feature_selection(self, X_train, y_train_col, X_test):
        ...
        try:
            ...
            print(f"    LASSO selection failed: {e}, using all features")
    ...
    def run_single_experiment(self, fold_num, model_name, feature_selection_name):
        ...
        try:
            ...
            for i, score_col in enumerate(self.score_columns):
                ...
                # --- FIX: REMOVED UNNECESSARY ADJUSTMENT FOR 0-INDEXED LABELS ---
                # y_train_col_adj = y_train_col - 1 if model_name == 'XGBoost' else y_train_col
                # The y_train_col is already [0, 1], which is perfect for all models, including XGBoost.
                ...
                if feature_selection_name == 'Baseline':
                    ...
                    X_train_selected, X_test_selected, support = self.apply_lasso_feature_selection(X_train_scaled,
                                                                                                    y_train_col_adj,
                                                                                                    # Now uses the correct y_train_col
                                                                                                    X_test_scaled)
                    ...
                    X_train_selected = selector.fit_transform(X_train_scaled,
                                                              y_train_col_adj)  # Now uses the correct y_train_col
                ...
                model_clone.fit(X_train_selected, y_train_col_adj)  # Now uses the correct y_train_col
                y_pred = model_clone.predict(X_test_selected)

                # --- FIX: REMOVED UNNECESSARY REVERSION ---
                # if model_name == 'XGBoost': y_pred += 1
                # Since we didn't subtract 1, we don't need to add it back.
                # --- END OF FIX ---
    ...
    def aggregate_results(self):
        """Aggregate results across folds for each target and for the aggregate"""
    ...
    def save_results(self):
        ...
        results_dir = self.base_dir / "comprehensive_results_binary"
        ...
        self.save_feature_analysis()
        print("Results saved successfully.")

    def save_feature_analysis(self):
        """Analyze and save detailed feature usage patterns."""
        feature_analysis_dir = self.base_dir / "comprehensive_results_binary" / "feature_analysis"
        feature_analysis_dir.mkdir(exist_ok=True)
        summary_rows = []

        for model_name, fs_data in self.feature_usage.items():
            for fs_name, score_data in fs_data.items():
                if fs_name in ['Baseline'] or fs_name.startswith('PCA'): continue
                for score_col, fold_data in score_data.items():
                    if not fold_data: continue
                    all_features = set(f for fd in fold_data for f in fd['used_features'] + fd['unused_features'])
                    usage_count = {f: sum(1 for fd in fold_data if f in fd['used_features']) for f in all_features}
                    total_folds = len(fold_data)

                    always = sorted([f for f, c in usage_count.items() if c == total_folds])
                    sometimes = sorted(
                        [(f, c, (c / total_folds) * 100) for f, c in usage_count.items() if 0 < c < total_folds],
                        key=lambda x: x[1], reverse=True)
                    never = sorted([f for f, c in usage_count.items() if c == 0])

                    summary_rows.append({
                        'Model': model_name, 'FeatureSelection': fs_name, 'ScoreColumn': score_col,
                        'TotalFeatures': len(all_features), 'AlwaysUsed': len(always),
                        'SometimesUsed': len(sometimes), 'NeverUsed': len(never)
                    })

                    method_dir = feature_analysis_dir / f"{model_name}_{fs_name}_{score_col}"
                    method_dir.mkdir(exist_ok=True)
                    if always:
                        with open(method_dir / "always_used.txt", 'w') as f:
                            f.write("\n".join(always))
                    if sometimes:
                        with open(method_dir / "sometimes_used.txt", 'w') as f:
                            f.write("\n".join(
                                [f"{name} ({count}/{total_folds}, {pct:.1f}%)" for name, count, pct in sometimes]))

        if summary_rows:
            pd.DataFrame(summary_rows).to_csv(feature_analysis_dir / "feature_usage_summary.csv", index=False)
        print("Feature analysis saved.")

    def create_visualizations(self):
        """Create comprehensive visualizations for each target and the aggregate."""
        results_dir = self.base_dir / "comprehensive_results_binary"
    ...
    def generate_report(self):
        ...
        with open(report_path, 'w') as f:
            f.write("COMPREHENSIVE MACHINE LEARNING PIPELINE REPORT\n")
            f.write("=" * 50 + "\n\n")
...
def main():
    ...
    base_dir = r"C:\Research\AI Folder\Thesis\Data\data_CTO_Kshitij\Main\10-fold-final-processed"
    ...
    print("Models will be trained for each target variable separately.")
    print("This may take a while due to the extensive combinations being tested.")
    ...
    print("Check the 'comprehensive_results_binary' directory for all outputs.")
