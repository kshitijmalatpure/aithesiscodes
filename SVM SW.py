...
# PCA settings
n_components = 0.95  # or set to an integer, e.g., 30
...
for fold_num in range(1, 11):
    ...
    output_path = fold_dir / f"fsSVM_PCA_Results_fold{fold_num}.txt"
    ...
    # Save explained variance plot for the first fold (after PCA)
    pca_plot_saved = False
    ...
    with open(output_path, "w") as f:
        ...
        for i, col in enumerate(score_column):
            ...
            # Save explained variance plot for the first fold and first target
            if fold_num == 1 and not pca_plot_saved:
                plt.figure(figsize=(8, 4))
                plt.plot(np.cumsum(pca.explained_variance_ratio_), marker='o')
                plt.xlabel('Number of Components')
                plt.ylabel('Cumulative Explained Variance')
                plt.title('PCA Explained Variance (Fold 1)')
                plt.grid(True)
                plt.tight_layout()
                plt.savefig(base_dir / "fsPCA_Explained_Variance_Fold1_PCA.png")
                plt.close()
                pca_plot_saved = True

            # SMOTE
            ros = SMOTE(k_neighbors=1, random_state=42)
            X_train_pca_res, y_train_col_res = ros.fit_resample(X_train_pca, y_train_col)

            clf = SVC(kernel='rbf', random_state=42)
            clf.fit(X_train_pca_res, y_train_col_res)
...
summary_csv_path = base_dir / "fsSVM_PCA_Metrics_Summary.csv"
...
agg_csv_path = base_dir / "fsSVM_PCA_Metrics_AggregatedSTD.csv"
agg_df.to_csv(agg_csv_path, index=False)

confmat_path = base_dir / "fsSVM_PCA_AllFolds_SummedConfusionMatrices.txt"
...
print(f"PCA explained variance plot saved to {base_dir / 'iPCA_Explained_Variance_Fold1_PCA.png'}")
