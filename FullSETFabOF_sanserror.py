...
def create_enhanced_confusion_matrix(cm, labels, title, save_path, method_name="", vmax=None):
    ...
    ax = sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=labels, yticklabels=labels, cbar_kws={'label': 'Count'}, vmax=vmax, square=True, linewidths=0.5, linecolor='white')
...
def save_confusion_matrix_data(cm, labels, save_path, method_name, score_col):
    cm_df = pd.DataFrame(cm, index=labels, columns=labels); cm_df.index.name = 'True_Label'; cm_df.columns.name = 'Predicted_Label'
    csv_path = save_path.parent / f"{save_path.stem}_data.csv"; cm_df.to_csv(csv_path)
    stats = calculate_cm_statistics(cm, labels); stats_df = pd.DataFrame([stats]); stats_df['Method'] = method_name; stats_df['Score_Column'] = score_col
...
def calculate_cm_statistics(cm, labels):
    ...
    for i, label in enumerate(labels):
        tp = cm[i, i]; fp = np.sum(cm[:, i]) - tp; precision = tp / (tp + fp) if (tp + fp) > 0 else 0; precision_per_class.append(precision)
        fn = np.sum(cm[i, :]) - tp; recall = tp / (tp + fn) if (tp + fn) > 0 else 0; recall_per_class.append(recall)
        f1 = 2 * (precision * recall) / (precision + recall) if (precision + recall) > 0 else 0; f1_per_class.append(f1)
    stats = {'Total_Predictions': total, 'Correct_Predictions': correct, 'Overall_Accuracy': accuracy, 'Macro_Precision': np.mean(precision_per_class), 'Macro_Recall': np.mean(recall_per_class), 'Macro_F1': np.mean(f1_per_class)}
    for i, label in enumerate(labels): stats[f'Precision_Class_{label}'] = precision_per_class[i]; stats[f'Recall_Class_{label}'] = recall_per_class[i]; stats[f'F1_Class_{label}'] = f1_per_class[i]
    return stats
...
class FabOFV840Enhanced:
    def __init__(self, base_dir, name_col, score_cols, labels, importance_threshold=0.005, features_per_page=15):
        ...
        self.root = self.base_dir / "fabOF_results_v8.4.0_TUNED_PIPELINE_FIXED_SKIP_LAST"
        ...
        for sc_idx, sc in enumerate(self.score_cols, 1):
            ...
            for fold in range(1, 11):
                ...
                try:
                    ...
                    # NEW: Exclude the last feature from both train and test sets
                    if len(feats) > 0:
                        feats = feats[:-1]  # Remove the last feature
                        self.logger.debug(f"    Excluded last feature. Using {len(feats)} features instead of {len(feats)+1}")
                    ...
                    rows.append(met)
                    self.logger.debug(f"    Fold metrics - Accuracy: {met['Accuracy']:.3f}, F1: {met['F1_macro']:.3f}, QWK: {met['QWK']:.3f}")
...
if __name__ == "__main__":
    ...
    importance_threshold = 0.01  # Features with importance >= 0.005 will be included
