...
for fold_num in range(1, 11):
    ...
    output_path = fold_dir / f"bGNB_Results_fold{fold_num}.txt"
    ...
    # Train and predict for each score column
    with open(output_path, "w") as f:
        print(f"\n{'=' * 20} Fold {fold_num} {'=' * 20}\n", file=f)
        for i, col in enumerate(score_column):
            y_true = y_test_int[:, i]
            y_train_col = y_train_int[:, i]
            # Train GaussianNB for this target
            clf = GaussianNB()
...
summary_csv_path = base_dir / "bGNB_Metrics_Summary.csv"
...
agg_csv_path = base_dir / "bGNB_Metrics_Aggregated.csv"
agg_df.to_csv(agg_csv_path, index=False)

# Save all summed confusion matrices to a text file
confmat_path = base_dir / "bGNB_AllFolds_SummedConfusionMatrices.txt"
