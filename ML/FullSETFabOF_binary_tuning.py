...
=======================================================
Hyperparameter tuning for FabOF (binary) with a simplified output layout.

Changes vs. MIN version:
- KBest is now TUNED via `k_best` (e.g., [50, 100, 150, 200, "all"])
- If k_best == "all" (or None), feature selection is skipped.
...
class HyperparameterSpace:
    ...
    def get_feature_selection_space(search_type: str = 'grid') -> Dict[str, List]:
        """
        k_best: number of features to keep. Use "all" (or None) to skip FS.
        """
        if search_type == 'grid':
            return {'k_best': [50, 100, 150, 200, "all"]}  # <-- no 75
        # random
        return {'k_best': [40, 60, 80, 100, 120, 150, 200, 250, "all"]}
...
class FabOFHyperparameterTuner:
    def __init__(self, base_dir: str, name_col: str, score_cols: List[str],
        ...
        self.logger.info("=" * 60)
        self.logger.info("FabOF Hyperparameter Tuning (BINARY, WITH KBest tuning)")
        self.logger.info("=" * 60)
    ...
    def _feature_select(
        ...
        """
        Apply SelectKBest if k_best is an int > 0 and less than n_features.
        If k_best == "all" or None or >= n_features, skip FS (return original).
        """
        k_param: Union[int, str, None] = params.get('k_best', "all")
        n_features = X_tr.shape[1]

        if isinstance(k_param, str):
            k_param = k_param.lower()
        if k_param in ("all", None):
            return X_tr, X_te
        ...
        selector = SelectKBest(f_regression, k=k_int)
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("FABOF HYPERPARAMETER TUNING (BINARY, WITH KBest tuning)")
    print("="*60)
