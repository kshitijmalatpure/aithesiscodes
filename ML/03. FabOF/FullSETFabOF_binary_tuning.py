"""
fabOF_v8_4_0_hyperparameter_tuning_BINARY_WITH_KBEST.py
=======================================================
Hyperparameter tuning for FabOF (binary) using Script 2's tuning structure.

CHANGES (per user request):
- KBest-only feature selection (no LASSO anywhere)
- Use Script 2's tuning structure and outputs
- Keep outputs minimal (no visualizations, confusion matrices, feature analysis)
- Root folder: comprehensive_fabOF_binary_tuning
  - logs/
  - aggregated_results/ (aggregated_tuning_results.csv, best_configurations.csv, best_params_snippet.txt)
...
class HyperparameterSpace:
    ...
    def get_preprocessing_space(search_type: str = 'grid') -> Dict[str, List]:
        # Match Script 2 behavior: tune imputation; scaling can be passed explicitly if desired.
        return {
...
class FabOFHyperparameterTuner:
    def __init__(self, base_dir: str, name_col: str, score_cols: List[str],
        ...
        # Output layout (Script 2 style)
        self.results_root = self.base_dir / "comprehensive_fabOF_binary_tuning"
        ...
        self.logger.info("=" * 60)
        self.logger.info("FabOF Hyperparameter Tuning (BINARY, KBest-only, Script 2 structure)")
        self.logger.info("=" * 60)
    ...
    def _feature_select(
        ...
        if method != 'kbest':
            method = 'kbest'  # enforce KBest-only
    ...
    # ---------- FabOF core (class-agnostic; handles 0/1 correctly) ----------
    def _fit_fabof(self, X_tr: np.ndarray, y_tr: np.ndarray, rf_params: Dict[str, Any]) -> Tuple[Any, np.ndarray, np.ndarray]:
        ...
        # Compute borders using cumulative class prevalences (works for binary 0/1 too)
        oob = rf.oob_prediction_
        classes_sorted = np.unique(y_tr)
        inner_classes = classes_sorted[:-1]
        pi = np.array([(y_tr <= c).mean() for c in inner_classes])  # cumulative up to each inner class
    ...
    def _predict_fabof(self, rf: Any, borders: np.ndarray, classes_sorted: np.ndarray, X_te: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _evaluate_params(self, params: Dict[str, Any], score_col: str,
        ...
        for fold in folds:
            try:
                ...
                rf, borders, classes_sorted = self._fit_fabof(X_tr, y_tr, params)
                y_pred, y_raw = self._predict_fabof(rf, borders, classes_sorted, X_te)
    ...
    def run_comprehensive_tuning(self) -> Dict[str, Dict[str, Any]]:
        ...
        for score_col in self.score_cols:
            ...
            # Build flat dict for the TXT snippet
        ...
        # BEST_PARAMS snippet for runner
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("FABOF HYPERPARAMETER TUNING (BINARY, KBest-only, Script 2 structure)")
    print("="*60)
    ...
    try:
        ...
        print(f" - {tuner.agg_dir / 'best_params_snippet.txt'}")
    except Exception as e:
