...
- Feature selection OPTIONS:
  ...
  * lasso (LassoCV with explicit alphas; |coef| > 1e-5; fallback top-50 by |coef|)
...
def _fit_selector(method: str, X_train_scaled: np.ndarray, y_train: np.ndarray):
    ...
    if method.startswith('kbest_'):
        ...
        sel.fit(X_train_scaled, y_train.astype(float))
        return sel

    elif method == 'lasso':
        # Aligned with classic: explicit alphas, stricter threshold, fallback 50
        lcv = LassoCV(alphas=np.logspace(-4, 4, 50), cv=5, random_state=42, max_iter=2000, n_jobs=-1)
        lcv.fit(X_train_scaled, y_train.astype(float))
        coefs = lcv.coef_
...
# ---------- FabOF core (binary; expects already selected arrays) ----------
...
def _calc_metrics(y_true, y_pred_raw, y_pred_bin, labels):
    base_metrics = {
        ...
        'F1-macro': f1_score(y_true, y_pred_bin, labels=labels, average='macro', zero_division=0),
        'Precision-macro': precision_score(y_true, y_pred_bin, labels=labels, average='macro', zero_division=0),
        'Recall-macro': recall_score(y_true, y_pred_bin, labels=labels, average='macro', zero_division=0),
...
def run_pipeline_for_selector(
    ...
    """
    Runs 10-fold pipeline for ONE selector across all binary score columns.
    ...
    for sc in score_cols:
        logger.info(f"=== Selector={selector_name} | Score={sc} (BINARY) ===")
        fold_metrics_list = []
        ...
        for fold in range(1, n_folds + 1):
            ...
            # Fit FabOF (binary)
            rf, borders = _fit_fabOF_selected_binary(X_train_sel, y_train, n_trees=500)

            # Predict
            y_pred_bin, y_pred_raw = _predict_fabOF_selected_binary(rf, borders, X_test_sel)

            # Metrics
            metrics = _calc_metrics(y_test, y_pred_raw, y_pred_bin, labels)
            fold_metrics_list.append(metrics)

            # store per-fold preds/truths
            per_fold_predictions[sc].append(y_pred_bin)
...
def save_aggregated_results(base_dir: Path, results_by_selector: Dict[str, Dict[str, Dict[str, float]]], score_cols):
    ...
    out_root = base_dir / "comprehensive_fabOF_binary" / "aggregated_results"
...
def save_best_confusion_matrices(base_dir: Path, best_selectors: Dict[str, str], score_cols, logger: logging.Logger, labels=(0,1)):
    out_dir = base_dir / "comprehensive_fabOF_binary" / "best_selector_confusion_matrices"
    out_dir.mkdir(parents=True, exist_ok=True)
    logger.info("Saving BEST-selector aggregated confusion matrices per score column (binary)...")
    ...
    for sc in score_cols:
        ...
        for fold in range(1, 11):
            ...
            rf, borders = _fit_fabOF_selected_binary(X_train_sel, y_train, n_trees=500)
            y_pred_bin, _ = _predict_fabOF_selected_binary(rf, borders, X_test_sel)

            cm = confusion_matrix(y_test, y_pred_bin, labels=labels)
            cm_total += cm

        # Save
        title = f"Aggregated Confusion Matrix (Best Selector: {sel_name}) - {sc} (Binary)"
...
def main():
    ...
    saving aggregated results, and saving one CM per score column for the best selector (binary).
    """
    ...
    # Prepare output dirs
    root = base_dir / "comprehensive_fabOF_binary"
    (root / "logs").mkdir(parents=True, exist_ok=True)
    logger = setup_logging(root / "logs")

    try:
        logger.info("Starting BINARY FabOF pipeline with multiple feature selectors...")
        ...
        logger.info(f"Best selectors per score column (binary): {best_selectors}")

        # Save aggregated results CSV (+ aggregate rows)
        df = save_aggregated_results(base_dir, results_by_selector, score_cols)
        logger.info(f"Aggregated results saved (binary). Shape: {df.shape if df is not None else None}")
        ...
        print("=" * 60)
        print("Outputs:")
        print("  • One aggregated confusion matrix per score column (BEST selector, binary)")
