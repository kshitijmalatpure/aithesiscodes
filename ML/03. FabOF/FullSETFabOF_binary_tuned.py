"""
fabOF_binary_tuned.py
========================================
Runs a tuned FabOF-style pipeline for four *binary* traits
(A_WOB, A_GRB, A_COB, A_TTB) using FIXED IN-SCRIPT PARAMETERS
(BEST_PARAMS dict below).

Key features
------------
- Uses 10-fold files: fold_{n}/fold{n}trb.csv and fold{n}teb.csv
- Preprocess (impute + scale) per best params
- Feature selection: FLEXIBLE per-trait via BEST_PARAMS:
    * feature_selection_method: 'kbest' or 'none'
    * kbest_k: integer (used if 'kbest')
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# FIXED IN-SCRIPT PARAMS (edit these per trait as you wish)
# Keys per trait:
#   imputation_strategy: 'median' | 'most_frequent' | 'mean'
#   scaling_method: 'standard' | 'robust' | 'minmax' | 'none'
#   n_estimators, max_depth, min_samples_split, min_samples_leaf,
#   max_features, bootstrap, random_state
#   feature_selection_method: 'kbest' or 'none'
#   kbest_k: integer (used when 'kbest')
...
def preprocess_and_fs(
    ...
    k_best: int
) -> Tuple[np.ndarray, np.ndarray, np.ndarray, SimpleImputer, Optional[Any], Optional[SelectKBest]]:
    """
    Flexible feature selection:
      - 'kbest': SelectKBest(f_regression, k=k_best)
    ...
    selector = None
    if str(feature_selection_method).lower() == "kbest":
        k = min(int(k_best), X_tr_s.shape[1])
...
class FabOFTunedRunnerBinary:
    def __init__(self, base_dir: str, name_col: str,
        ...
        save_session_config(self.dirs, {
            ...
            "best_params_source": "internal_FIXED_BEST_PARAMS_dict"
        })
    ...
    def _trait_params(self, score_col: str) -> Dict[str, Any]:
        ...
        return {**defaults, **BEST_PARAMS.get(score_col, {})}
    ...
    def run(self):
            ...
            for fold in range(1, 11):
                try:
                    ...
                    X_tr_fs, X_te_fs, mask, imputer, scaler, selector = preprocess_and_fs(
                        ...
                        p.get('feature_selection_method', 'kbest'),
                        p.get('kbest_k', 50)
                    )
        ...
        with open(self.dirs["reports"] / "final_report.md", "w", encoding="utf-8") as f:
            ...
            f.write(f"- Params source: internal FIXED BEST_PARAMS dict\n")
