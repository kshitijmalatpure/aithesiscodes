"""
fabOF_v8_4_0_tuned_pipeline_FIXED_WITH_BEESWARM.py
=================================================
FIXED version to properly generate SHAP analysis and aggregated predictions
WITH ENHANCED BEESWARM PLOTS for features above importance threshold
"""
...
# Central dictionary for all best hyperparameters
BEST_PARAMS = {
    'A_WO': {
...
def create_enhanced_confusion_matrix(cm, labels, title, save_path, method_name="", vmax=None):
    ...
    ax = sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=labels, yticklabels=labels,
                     cbar_kws={'label': 'Count'}, vmax=vmax, square=True, linewidths=0.5, linecolor='white')
...
def save_confusion_matrix_data(cm, labels, save_path, method_name, score_col):
    ...
    stats = calculate_cm_statistics(cm, labels);
...
def calculate_cm_statistics(cm, labels):
    ...
    stats = {'Total_Predictions': total, 'Correct_Predictions': correct, 'Overall_Accuracy': accuracy,
             'Macro_Precision': np.mean(precision_per_class), 'Macro_Recall': np.mean(recall_per_class),
             'Macro_F1': np.mean(f1_per_class)}
    for i, label in enumerate(labels): stats[f'Precision_Class_{label}'] = precision_per_class[i]; stats[
        f'Recall_Class_{label}'] = recall_per_class[i]; stats[f'F1_Class_{label}'] = f1_per_class[i]
    return stats
...
def create_shap_beeswarm_plots(shap_values, X_data, feature_names, fs_name, sc, save_dir,
    ...
    """
    Create SHAP beeswarm plots for features above importance threshold

    Parameters:
    -----------
    shap_values : numpy.ndarray
        Combined SHAP values for all samples
    X_data : numpy.ndarray
        Combined feature data for all samples
    feature_names : numpy.ndarray
        Array of feature names
    fs_name : str
        Feature selection method name
    sc : str
        Score column name (trait)
    save_dir : Path
        Directory to save plots
    importance_threshold : float
        Minimum feature importance to include (default: 0.005)
    features_per_page : int
        Maximum features per plot (default: 15)
    ...
    try:
        ...
        # Create importance dataframe and filter by threshold
        ...
        if len(significant_features) == 0:
            if logger:
                logger.warning(f"    No features above threshold {importance_threshold} for {sc}")
            return

        if logger:
            logger.info(f"    Found {len(significant_features)} features above threshold for {sc}")
        ...
        for page in range(n_pages):
            ...
            # Extract SHAP values and data for selected features
            ...
            try:
                ...
                if logger:
                    logger.info(f"    Saved beeswarm plot page {page + 1}/{n_pages}: {filename}")

            except Exception as plot_error:
                if logger:
                    logger.warning(f"    Failed to create beeswarm plot page {page + 1}: {str(plot_error)}")
                plt.close()
                continue

        # Save feature importance summary for significant features
        ...
        if logger:
            logger.info(f"    Saved significant features summary: {summary_path.name}")

    except Exception as e:
        if logger:
            logger.error(f"    Error creating beeswarm plots for {sc}: {str(e)}")


# ---------- Helper Functions ----------
def _fit_fabOF(X_train, y_train, params):
    # 1. Dynamic Imputation
    ...
    # 3. Dynamic RandomForestRegressor parameters
    rf_params = {
    ...
    pi = np.array([(y_train <= c).mean() for c in range(1, k)])
    borders_inner = np.quantile(oob_pred, pi)
    borders = np.concatenate([[1.0], borders_inner, [float(k)]])

    return rf, borders, scaler, imputer, rf.feature_importances_
...
def _predict_fabOF(rf, borders, X, scaler, imputer):
    ...
    y_ord = np.searchsorted(borders[1:-1], y_num) + 1
    return y_ord
...
class FabOFV840Enhanced:
    def __init__(self, base_dir, name_col, score_cols, labels, importance_threshold=0.005, features_per_page=15):
        ...
        self.importance_threshold = importance_threshold  # NEW: Importance threshold for beeswarm plots
        self.features_per_page = features_per_page  # NEW: Features per beeswarm plot page
        self.root = self.base_dir / "fabOF_results_v8.4.0_TUNED_PIPELINE_FIXED_ERRORONLY"
        ...
        self.dirs = {
            ...
            'shap_beeswarm': self.root / 'shap_beeswarm_plots',  # NEW: Directory for beeswarm plots
        ...
        self.logger.info("=" * 60)
        self.logger.info("FabOF v8.4.0 - FIXED TUNED PIPELINE with SHAP and Beeswarm Plots")
        self.logger.info("=" * 60)
    ...
    def run(self):
        """Main execution method using tuned hyperparameters for each score column."""
        ...
        # Loop over each target trait (score column)
        for sc_idx, sc in enumerate(self.score_cols, 1):
            self.logger.info(
                f"\n[{sc_idx}/{len(self.score_cols)}] Processing Score Column: {sc} with its optimal parameters")
            sc_start_time = time.time()

            # Fetch the specific hyperparameters for this score column
            try:
            ...
            # Reset aggregators for each score column
            ...
            for fold in range(1, 11):
                ...
                try:
                    ...
                    # NEW: Use ONLY the last feature from both train and test sets
                    if len(feats) > 0:
                        last_feature = feats[-1]  # Get the last feature
                        feats = [last_feature]  # Use only the last feature
                        self.logger.debug(f"    Using ONLY the last feature: {last_feature}")
                    ...
                    rf, borders, scaler, model_imputer, imp = _fit_fabOF(X_tr_fs, y_tr, params)

                    y_pred_rounded = _predict_fabOF(rf, borders, X_te_fs, scaler, model_imputer)
                    y_pred_raw = rf.predict(
                        scaler.transform(model_imputer.transform(X_te_fs)) if scaler else model_imputer.transform(
                            X_te_fs))
                    ...
                    fold_predictions_df.to_csv(self.dirs['predictions'] / f"{fs_name}_{sc}_fold{fold}_predictions.csv",
                                               index=False)
                    ...
                    # Calculate metrics
                    met = _calc_metrics(y_te, y_pred_raw, y_pred_rounded, self.labels)
                    met.update({'FS_Method': fs_name, 'Score_Column': sc, 'Fold': fold})
                    rows.append(met)
                    self.logger.debug(
                        f"    Fold metrics - Accuracy: {met['Accuracy']:.3f}, F1: {met['F1_macro']:.3f}, QWK: {met['QWK']:.3f}")

                    # Confusion matrix
                    fold_cm = confusion_matrix(y_te, y_pred_rounded, labels=self.labels)
                    ...
                    # FIXED: Complete SHAP analysis
                    self.logger.debug("    Starting SHAP analysis...")
                    try:
                        explainer = shap.TreeExplainer(rf)
                        X_te_fs_processed = scaler.transform(
                            model_imputer.transform(X_te_fs)) if scaler else model_imputer.transform(X_te_fs)
                        shap_values = explainer.shap_values(X_te_fs_processed)
                        all_shap_values.append(shap_values)
                        all_X_te_fs.append(X_te_fs_processed)

                        # Save individual fold SHAP plots
                        if len(shap_values) > 0:
                            # Create summary plot for this fold
                            plt.figure(figsize=(10, 8))
                            shap.summary_plot(shap_values, X_te_fs_processed, feature_names=selected_feature_names,
                                              show=False)
                            ...
                            if len(X_te_fs_processed) > 0:
                                ...
                                plt.tight_layout()
                                plt.savefig(
                                    self.dirs['shap_per_fold'] / f"{fs_name}_{sc}_fold{fold}_shap_waterfall.png",
                                    dpi=300, bbox_inches='tight')
                                plt.close()
            ...
            # Save aggregated predictions for this score column
            if all_predictions:
            ...
            try:
                self._save_combination_results(fs_name, sc, rows, cms, rf_imp, feature_masks,
        ...
        # Save master aggregated predictions file combining all score columns
    ...
    def _save_combination_results(self, fs_name, sc, rows, cms, rf_imp, feature_masks,
        ...
        if not df_fold.empty:
            ...
            agg_stats = df_fold[metrics_cols].agg(['mean', 'std', 'min', 'max']).T.reset_index().rename(
        ...
        if rf_imp and all_feature_names is not None:
            ...
            importance_df.to_csv(self.dirs['feature_importance'] / f"{fs_name}_{sc}_feature_importance.csv",
                                 index=False)
        ...
        # ENHANCED: SHAP analysis with beeswarm plots
        if all_shap_values and any(sv is not None for sv in all_shap_values):
            try:
                ...
                if valid_shap_values and valid_X_te_fs and all_feature_names is not None:
                    ...
                    # NEW: Create SHAP beeswarm plots for important features
                    ...
                    for i, feature_idx in enumerate(top_5_indices):
                        try:
                            ...
                            plt.tight_layout()
                            plt.savefig(self.dirs[
                                            'shap_analysis'] / f"{fs_name}_{sc}_shap_dependence_{all_feature_names[feature_idx]}.png",
                                        dpi=300, bbox_inches='tight')
                            plt.close()
                        except Exception as dep_error:
                            self.logger.warning(
                                f"    Could not create dependence plot for feature {feature_idx}: {str(dep_error)}")
                    ...
                    shap_importance.to_csv(self.dirs['shap_analysis'] / f"{fs_name}_{sc}_shap_importance.csv",
                                           index=False)
    ...
    def _save_all_confusion_matrices(self):
        ...
        # Individual enhanced confusion matrices
...
if __name__ == "__main__":
    ...
    # NEW: Beeswarm plot configuration
    importance_threshold = 0.005  # Features with importance >= 0.005 will be included
    features_per_page = 15  # Maximum 15 features per beeswarm plot
    ...
    try:
        pipeline = FabOFV840Enhanced(
            ...
            labels=labels,
            importance_threshold=importance_threshold,  # NEW
            features_per_page=features_per_page  # NEW
        )
        pipeline.run()
        print("\n" + "=" * 60)
        print("FIXED TUNED PIPELINE WITH BEESWARM PLOTS COMPLETED!")
        print("=" * 60)
        print(f"Results saved to: {pipeline.root}")
        print("\nGenerated outputs:")
        print("✓ Individual fold predictions")
        print("✓ Aggregated predictions per score column")
        print("✓ Master aggregated predictions file")
        print("✓ SHAP analysis (summary plots, dependence plots, feature importance)")
        print("✓ SHAP beeswarm plots for important features (threshold ≥ 0.005)")
