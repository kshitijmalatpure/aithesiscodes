"""
fabOF_v8_4_0_fourclass_with_feature_selection.py
=================================================
...
class FabOFV840FourClassWithFS:
    """
    4-class pipeline with feature selection:
    ...
    def __init__(self, base_dir, name_col, score_cols, labels,
        ...
        # Log init
        self.logger.info("=" * 60)
        self.logger.info("FabOF v8.4.0 4-Class Classification WITH Feature Selection")
        self.logger.info("=" * 60)
    ...
    def run(self):
        self.logger.info("Starting pipeline with feature selection...")
            ...
            for fold in range(1, 11):
                for method in self.selectors:
                    ...
                    try:
                        ...
                        # FabOF training/prediction on selected features
                        rf, borders = _fit_fabOF_selected(X_tr_sel, y_tr)
                        y_pred_round, y_pred_raw = _predict_fabOF_selected(rf, borders, X_te_sel)
                        ...
                        selector_predictions[method]["pred"].extend(y_pred_round.tolist())
        ...
        self.logger.info("=" * 60)
        self.logger.info("FabOF 4-Class WITH FS Pipeline COMPLETED")
        self.logger.info("=" * 60)
    ...
    def _save_aggregated_results(self, all_rows):
        """
        Save a single CSV with aggregated metrics.
        - Rows per (Score_Column, Selector): mean/std across folds
        - PLUS: one AGGREGATE row per Selector averaging the per-trait means across traits
                (std reported across traits).
        """
        if not all_rows:
        ...
        trait_list = list(self.score_cols)  # e.g., ['A_WO','A_GR','A_CO','A_TT']
        df_trait_means = (
            df[df['Score_Column'].isin(trait_list)]
            .groupby(['Score_Column', 'Selector'])[metrics_cols]
            .mean()  # fold means per (trait, selector)
            .reset_index()
        )

        for sel, d in df_trait_means.groupby('Selector'):
            agg_sel = {f'{m}_mean': d[m].mean() for m in metrics_cols}  # average across traits
            agg_sel.update({f'{m}_std': d[m].std(ddof=0) for m in metrics_cols})  # std across traits
...
if __name__ == "__main__":
    ...
    try:
        pipeline = FabOFV840FourClassWithFS(base_dir, name_col, score_cols, labels, selectors)
        pipeline.run()
